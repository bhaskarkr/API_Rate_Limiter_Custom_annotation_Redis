...
public class RateLimitCacheImpl implements RateLimitCache {
    ...
    @Override
    public Optional<Integer> get(String userId) {
        Integer count = limitValueMapCache.getOrDefault(userId, null);
        if(Objects.nonNull(count) && count != -1) {
            log.info("TTL left {}", Math.abs(limitValueMapCache.remainTimeToLive(userId))/1000);
    ...
    @Override
    public void put(String userId, long ttl) {
        Optional<Integer> currCount = get(userId);
        if(!currCount.isEmpty())
            // it returns time in millis
            ttl = Math.abs(limitValueMapCache.remainTimeToLive(userId))/1000;
        log.info("new TTL  {}  {}", ttl, cacheNameConfig.getTtlType().name());
        limitValueMapCache.put(userId, currCount.orElse(1) + Integer.valueOf(1), ttl, cacheNameConfig.getTtlType());
    }
}
